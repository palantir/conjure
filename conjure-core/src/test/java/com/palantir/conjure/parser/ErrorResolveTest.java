...
public class ErrorResolveTest {

    private final ObjectMapper objectMapper = new ObjectMapper().registerModule(new Jdk8Module());

    @Test
    public void resolve_errors_in_ir() throws IOException {
        ConjureDefinition normalized =
                Conjure.parse(ImmutableSet.of(new File("src/test/resources/resolve-errors.yml")));

        String actual = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(normalized);

        File file = new File("src/test/resources/resolved-errors.conjure.json");
