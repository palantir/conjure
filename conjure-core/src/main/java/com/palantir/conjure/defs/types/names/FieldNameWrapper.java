...
public final class FieldNameWrapper {
    ...
    public static FieldName fieldName(String fieldName) {
        validate(fieldName);
    ...
    public static void validate(String fieldName) {
        Preconditions.checkArgument(
                Case.LOWER_CAMEL_CASE.pattern.matcher(fieldName).matches()
                        || Case.KEBAB_CASE.pattern.matcher(fieldName).matches()
                        || Case.SNAKE_CASE.pattern.matcher(fieldName).matches(),
                "FieldName \"%s\" must follow one of the following patterns: %s",
                fieldName, Arrays.toString(Case.values()));

        if (!Case.LOWER_CAMEL_CASE.pattern.matcher(fieldName).matches()) {
            log.warn("{} should be specified in lowerCamelCase. kebab-case and snake_case are supported for "
                    + "legacy endpoints only: {}", FieldName.class, fieldName);
        }
    }
